...
library(here)
library(gt)
mydata <- read.csv("data/personal_data.csv")
```
...
##c.) 
     [Average time spent on school work(min) measured each day of the week Sunday-Friday. Open circles represent the individual observations for time spent on school work each day. The red point represents the mean time spent on school work]
...
mean_time = "Mean(cm)") |> # changes column name
