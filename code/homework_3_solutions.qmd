...
The authors dont privide a figure/table for the specific Kruskal Wallis test above. However one other Kruskal Wallis test the authors run is on the source pond type and how this affects mp concentrations in water and sediment. The response variable is the source pond type being saltwater and a mixture of saltwater and river water. The predictor variable is mp concentration in both water and sediment. The table provided is missing some components to  clearly represent the data underlying the test. It accurately proved the p-value however the ranking(H value) of the Kruskal Wallis test is not provided and this would be important to provide because the main part for a Kruskal Wallis test is comparing the ranks between groups and a H value would tell us the magnitude of the difference between groups.


## c. Aesthetic clarity
the authors could have included in the table some horizontal lines seprating the different groups of water and sediment. This would make it more organized and less clutered. In addition he could have organized the data and have a column named Kruskal Wallis instead of repeating it twice for each group. Lastly, having column names for the sample types would look more organized than having a long title explaning the different samples in parenthesis.


## c. Recommendations
Some recommendatoins I have for the table wpuld be to organize and format more neatly the table. For example having the kruskal wallice test not mentioned twice in the data part of the table. I would aslo recommend to add more than just the p-value from the test. Since the author is running a Kruskal Wallis test, they should add the H value or the result from the test to the data. In addition the degree of freedom should be mentioned because we want to know how many groups were compared and by only listing two of the groups in the sample title its hard to tell if 3 groups were compared.
